...
function clear_server(){
    ...
    if [ -d "$DIRECTORY/tserver/data/${CONFIG['server_name']}" ]; then
        echo "Remove $DIRECTORY/tserver/data/${CONFIG['server_name']}."
        rm -Rv $DIRECTORY/tserver/data/${CONFIG['server_name']}
    else
...
function run(){
    ...
    data_dir=$DIRECTORY/tserver/data/${CONFIG['server_name']}
    mkdir -p $data_dir
    echo "Server's data directory: $data_dir."

    echo "Running server."
    cd $DIRECTORY/tserver/
...
function backup(){
    ...
    if [ -d "$DIRECTORY/tserver/data/${CONFIG['server_name']}" ]; then
        cp -R $DIRECTORY/tserver/data/${CONFIG['server_name']} $DIRECTORY/backups/${CONFIG['server_name']}
        echo "Copy is saved to: $DIRECTORY/backups/${CONFIG['server_name']}"
    else
        echo -e "Server's directory $DIRECTORY/tserver/data/${CONFIG['server_name']}
...
function list_all_open_server(){
    ...
    \nTo stop server type following command: 'kill PID'"
